...
final class TabWordsViewModel: ObservableObject {
    ...
    func getWords(search: String = "") {
        ...
        if Int.random(in: 1...10) <= 1 {
            ...
            let error = AppError(
                errorType: .network,
                errorMessage: "Failed to load words due to network issues",
                additionalInfo: ["Context": "TabWordsViewModel"]
            )
            ErrorManager.shared.setError(appError: error, context: .words)
            return
        }
