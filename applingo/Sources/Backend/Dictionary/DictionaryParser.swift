...
final class DictionaryParser: ProcessDatabase {
   ...
   /// Manager responsible for dictionary database operations
   private let dictionaryManager: DatabaseManagerDictionary
   
   /// Manager responsible for word database operations
   ...
   override init() {
       ...
       self.dictionaryManager = DatabaseManagerDictionary(dbQueue: dbQueue)
       self.wordManager = DatabaseManagerWord(dbQueue: dbQueue)
       super.init()
   ...
   func importDictionary(
       ...
       // Этап 1: Парсинг файла (без транзакции)
       performDatabaseOperation(
           { [weak self] () throws -> (ParserModelDictionary, [ParserModelWord]) in
               ...
               // Попытка доступа к security-scoped ресурсу
           ...
           success: { [weak self] parseResult in
               ...
               // Этап 2: Сохранение данных в транзакции (атомарно)
               self.performDatabaseTransactionOperation(
                   { db in
                       try ParserManagerSave(
                           processDatabase: self,
                           dictionaryManager: self.dictionaryManager,
                           wordManager: self.wordManager
                   ...
                   success: { _ in
                       ...
                       // Уведомляем наблюдателей об успешном импорте
           ...
           completion: { result in
               // Если ошибка произошла на этапе парсинга, возвращаем её
